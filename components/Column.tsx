  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <div
          key={todo.title}
          className="bg-white rounded-md w-[280px] px-4 py-6 flex flex-col gap-2 shadow-lg hover:cursor-pointer"
        >
          <p className="text-black text-base font-bold">{todo.title}</p>
...
const SubtaskItem = ({ subtask }: { subtask: Subtask }) => {
  ...
  return (
    <div
      className="flex gap-4 bg-light-grey rounded-sm pl-3 py-4 items-center hover:bg-purple-hover hover:cursor-pointer"
      onClick={() => setIsChecked((c) => !c)}
    >
      <Checkbox
        className="w-4 data-[state=checked]:bg-main-purple data-[state=checked]:border-0 px-1"
        checked={isChecked}
      />
      <p
        className={cn(
          "text-dark-grey font-bold ",
